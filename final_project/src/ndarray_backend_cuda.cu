...
__global__ void MatmulKernel(const scalar_t* a,const scalar_t* b, scalar_t* out, uint32_t M , uint32_t P, uint32_t N)
{
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  MatmulKernel<<<grid, block>>>(a.ptr, b.ptr, out->ptr, M, P, N);
  /// END YOUR SOLUTION
}
...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  ReduceMaxKernel<<<grid, block>>>(a.ptr, out->ptr, reduce_size);
  /// END YOUR SOLUTION
}
