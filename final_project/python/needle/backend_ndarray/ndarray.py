...
class NDArray:
    ...
    def __matmul__(self, other):
        ...
        # print(self.numpy()[0:8,0:8])
        # print(other.compact().numpy()[0:8,0:8])
    ...
    def flip(self, axes):
        ...
        for i in axes:
            offset += strides[i] * (self.shape[i]-1)
            strides[i] *= -1
        strides = tuple(strides)
        a = NDArray.make(self.shape,strides,device = self.device,handle=self._handle,offset=offset).compact()
        return a
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        for i in range(len(shape)):
            shape[i] += axes[i][0]+axes[i][1]
        shape = tuple(shape)
        a = NDArray.make(shape,device = self.device)
        a.fill(0)
        idx = []
        for i in range(len(shape)):
