...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        # print("now node:",node.op)
        # for x in node_to_output_grads_list[node]:
        #    print(x.op) 
        # print(output_grad.op)
        node.grad = output_grad
        if node.op == None:
            continue
        grad = node.op.gradient_as_tuple(output_grad,node)
