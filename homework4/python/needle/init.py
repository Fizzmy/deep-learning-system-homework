...
def xavier_normal(fan_in, fan_out, shape=None, gain=1.0, **kwargs):
    ### BEGIN YOUR SOLUTION
    std = gain * pow(2 / (fan_in+fan_out), 0.5)
    if shape is None:
...
def kaiming_uniform(fan_in, fan_out, nonlinearity="relu", device=None, **kwargs):
    assert nonlinearity == "relu", "Only relu supported currently"
    ### BEGIN YOUR SOLUTION

    bound = pow(2,0.5) * pow(3 / fan_in, 0.5)
    if ("shape" in kwargs):
        return rand(*kwargs["shape"], low = -bound, high = bound, device=device)
    return rand(*(fan_in,fan_out), low = -bound, high = bound, device=device)
    ### END YOUR SOLUTION


def kaiming_normal(fan_in, fan_out, nonlinearity="relu",device=None, **kwargs):
    assert nonlinearity == "relu", "Only relu supported currently"
    ### BEGIN YOUR SOLUTION
    std = pow(2,0.5) * pow(1 / fan_in, 0.5)
    if ("shape" in kwargs):
        return rand(*kwargs["shape"], std = std, device=device)
    return randn(*(fan_in,fan_out), std = std, device=device)
    ### END YOUR SOLUTION
